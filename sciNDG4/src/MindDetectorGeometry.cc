// ----------------------------------------------------------------------------
//  $Id: MindDetectorGeometry.cc 467 2011-11-25 16:00:27Z ryan $
//
...
MindDetectorGeometry::MindDetectorGeometry()
{
  SetInputParameters();
...
void MindDetectorGeometry::SetInputParameters()
{
  ...
  for (G4int ilayer = 1;ilayer <= _number_active+1;ilayer++){
    ...
    if ( config.PeekDParam("gap"+gap_num) ){
      ...
      G4Exception("MindDetectorGeometry::SetInputParameters",
  ...
  _detector_length = config.GetDParam("length") * mm;
  _number_pieces = (G4int)ceil(_detector_length / _piece_length);
  _detector_length = _number_pieces * _piece_length;
...
G4LogicalVolume* MindDetectorGeometry::DefineDetector()
{
  // PIECE ..........................................................
  ...
  */
  ...
  G4PVReplica* piece_physi = 
    new G4PVReplica("PIECE", piece_logic, detector_logic, 
  ...
  */
  ...
  detector_logic->SetVisAttributes(red);
...
void MindDetectorGeometry::SetNullField(G4LogicalVolume& detector_logic)
{
...
void MindDetectorGeometry::SetMagneticField(G4LogicalVolume& detector_logic)
{
  // apply a global uniform magnetic field along Y axis
  G4FieldManager* fieldMgr = new G4FieldManager();
  ...
  if (fieldValue.size() != 0) { 
    if ( _isUniform ) {
      ...
      _magField = new G4UniformMagField(G4ThreeVector(fieldScaling*fieldValue[0]*tesla, 
    ...
    else if(config.PeekSParam("FieldMap")){
...
G4ThreeVector MindDetectorGeometry::GetVertex(G4String region)
{
  // A calorimeter piece is selected randomly
  G4int piece_no = (G4int)ceil(_number_pieces * G4UniformRand());
  ...
  if (region == "PASSIVE") {
    ...
    G4ThreeVector vtx = 
      RandomPointInBox(_piece_width, _piece_height, _passive_thickness);
    ...
    G4double Z = vtx.z() + _passive_displacement + 
      piece_no * _piece_length - _piece_length/2. - _detector_length/2.;
    ...
    G4ThreeVector vtx = 
      RandomPointInBox(_piece_width, _piece_height, _active_thickness);
    ...
    G4Exception("MindDetectorGeometry::GetVertex",
...
G4ThreeVector MindDetectorGeometry::RandomPointInBox(G4double width, 
						     G4double height,
						     G4double length)
{
...
G4ThreeVector MindDetectorGeometry::GaussianBeamSpot(G4ThreeVector origin,
						     G4double RMSx,
						     G4double RMSy){
...
void MindDetectorGeometry::CalculatePassiveTargetProb()
{
  ...
  _passive_target_prob = (pass_A * pass_dens * pass_th * act_M) /
    (pass_A * pass_dens * pass_th * act_M + act_A * act_dens * act_th * pass_M);
}

G4int MindDetectorGeometry::GetRegion(G4double zpos)
{
