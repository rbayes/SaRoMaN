				     ...
				     const std::multimap<int,bhep::hit*>& map1)
{
  ...
  if(OctGeom==0 && z > (_vertexDetdepth - _detectorLength)/2.){
    // Assume a cylindrical geometry
    double xedge = _detectorX * sqrt(1 - 4.*pow(voxY/_detectorY, 2))/2.;
    double yedge = _detectorY * sqrt(1 - 4.*pow(voxX/_detectorX, 2))/2.;
  ...
  else if(OctGeom==0 && z < (_vertexDetdepth - _detectorLength)/2.){
    // a cuboid geometry for a forward vertex.
    double xedge = _vertexDetX/2.;
    double yedge = _vertexDetY/2.;
    xE1 = xE1 * exp( -(xedge - fabs(voxX))/5000 );
    xE2 = xE2 * exp( -(xedge + fabs(voxX))/5000 );
    yE1 = yE1 * exp( -(yedge - fabs(voxY))/5000 );
    yE2 = yE2 * exp( -(yedge + fabs(voxY))/5000 );
