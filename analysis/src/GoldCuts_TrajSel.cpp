...

{
  ...
  inTree.SetBranchStatus("hadronHits",1);       inTree.SetBranchAddress("hadronHits", &_hadhits);
  ...
  inTree.SetBranchStatus("hadTrueE", 1);       inTree.SetBranchAddress("hadTrueE", &_hadEng);
...
bool GoldCuts_TrajSel::full_cut(){
  ...
  if ( fabs( _errqP[_mutraj] / _recqP[_mutraj] ) >= _errSig->GetXaxis()->GetXmax()) return false;
    ...
    double recEng = fabs(0.001/_recqP[_mutraj]);
    double dxdz = _recXdir[_mutraj];
    double dydz = _recYdir[_mutraj];
    double mP  = 938.27 * MeV;
    double mmu = 105.65 * MeV;
    double mN  = 939.56 * MeV;
    double Emu = sqrt(pow(1./_recqP[_mutraj],2) + mmu*mmu);
    double costhmu = 1./sqrt(1. + dxdz*dxdz + dydz*dydz);
    if(_recqP[_mutraj] > 0) // assume the QE interaction numubar + p \to n + mu^{+}                                                                                                                       
      _hadQESEng[0] = (mP * Emu + 0.5*(mN*mN - mmu*mmu - mP*mP))/(mP - Emu + costhmu/fabs(_recqP[_mutraj]));
    else if(_recqP[_mutraj] < 0) // assume the QE interaction numu + n \to p + mu^{-}                                                                                                                     
      _hadQESEng[0] = (mN * Emu + 0.5*(mP*mP - mmu*mmu - mN*mN))/(mN - Emu + costhmu/fabs(_recqP[_mutraj]));
    else
      _hadQESEng[0] = 0.;
    ...
    if(_hadhits == 0)
      ...
      double altEng = _enu - sqrt(1./_truqP/_truqP + mmu*mmu);
      if(altEng > 0) altEng *= 0.001;
      else altEng = 0.00;
	// 0.001 * sqrt( pow(_hadMom[0],2)+pow(_hadMom[1],2)+pow(_hadMom[2],2));
