...
Run_NDTMVA::Run_NDTMVA(const bhep::gstore& store)
...
*/  
{
  ...
  _piTrackreader = new TMVA::Reader( "!Color:!Silent" );
  // _piShowerreader = new TMVA::Reader( "!Color:!Silent" );
  _electronShowerreader = new TMVA::Reader( "!Color:!Silent" );
...
void Run_NDTMVA::set_MVA_methods(const bhep::gstore& store)
{
...
void Run_NDTMVA::define_cuts(const bhep::gstore& store){
  ...
  if(store.find_vstore("piMVAcut"))
    _piMVAcut     = store.fetch_vstore("piMVAcut");
  if(store.find_vstore("showerMVAcut"))
    _showerMVAcut = store.fetch_vstore("showerMVAcut");
...
/********************************************/
void Run_NDTMVA::define_variables(const bhep::gstore& store)
/********************************************/
{
  // Now add variables to the reader
  // In principle this can be controlled by the configuration file as well.
  vector<int> selvars;
  vector<int> piselvars;
  vector<int> showerselvars;
  if(store.find_ivstore("MVAVars"))
    selvars = store.fetch_ivstore("MVAVars");
  else { 
    for(int i=0; i<11; i++) selvars.push_back(1);
    selvars[3] = 0;
    selvars[6] = 0;
  }
  if(store.find_ivstore("piMVAVars"))
    piselvars = store.fetch_ivstore("piMVAVars");
  else { 
    for(int i=0; i<11; i++) piselvars.push_back(1);
    piselvars[3] = 0;
    piselvars[6] = 0;
  }
  if(store.find_ivstore("showerMVAVars"))
    showerselvars = store.fetch_ivstore("showerMVAVars");
  else { 
    for(int i=0; i<11; i++) showerselvars.push_back(1);
    showerselvars[3] = 0;
    showerselvars[6] = 0;
  }
  ...
  if(selvars[4]) _reader->AddVariable( "meanDep := trajEngDep[0]==trajEngDep[0] ? trajEngDep[0] : 0.0", &meanDep);
  if(selvars[5]) _reader->AddVariable( "EngVar := trajHighDep[0] > 0 && trajHighDep[0] == trajHighDep[0] ? trajLowDep[0]/trajHighDep[0] : 0.0", &EngVar);   
  ...
  if(selvars[9]) _reader->AddVariable( "showerHits := hadronHits", &showerHits);
  if(selvars[10]) _reader->AddVariable( "Range  := rangqP[0]*0.001", &rngMom);


  _piTrackreader->AddSpectator( "nuEnergy  := NeuEng * 0.001", &nuEnergy);
  if(piselvars[0]) _piTrackreader->AddVariable( "ErrqP  := ErrMom[0]/RecMom[0]", &ErrqP);
  if(piselvars[1]) _piTrackreader->AddVariable( "trHits := InMu[0]", &trHits);
  if(piselvars[2]) _piTrackreader->AddVariable( "Rp := initrangqP[0]/RecMom[0]", &Rp);
  if(piselvars[3]) _piTrackreader->AddVariable( "engfrac := visEngTraj[0]/visibleEng", &engfrac);
  if(piselvars[4]) _piTrackreader->AddVariable( "meanDep := trajEngDep[0]==trajEngDep[0] ? trajEngDep[0] : 0.0", &meanDep);
  if(piselvars[5]) _piTrackreader->AddVariable( "EngVar := trajHighDep[0] > 0 && trajHighDep[0] == trajHighDep[0] ? trajLowDep[0]/trajHighDep[0] : 0.0", &EngVar);   
  if(piselvars[6]) _piTrackreader->AddVariable( "recMom    := abs( 0.001/RecMom[0] )", &recMom);
  // if(piselvars[7]) _piTrackreader->AddVariable( "Qt  :=  abs( 0.001/RecMom[0] ) * ( 1 - pow( hadRdirP[0][0]*RecXDirection[0] + hadRdirP[0][1]*RecYDirection[0] + hadRdirP[0][2], 2)/(1 + pow(RecXDirection[0],2) + pow(RecYDirection[0],2)))", &Qt );
  if(piselvars[7]) _piTrackreader->AddVariable( "Qt  :=  abs( 0.001/RecMom[0] ) * ( 1 - pow(RecShowerDir[0]*RecXDirection[0] + RecShowerDir[1]*RecYDirection[0] + RecShowerDir[2], 2)/(1 + pow(RecXDirection[0],2) + pow(RecYDirection[0],2)))", &Qt );
  if(piselvars[8]) _piTrackreader->AddVariable( "meanHPP := Nallhits/Planes", &meanHPP);
  if(piselvars[9]) _piTrackreader->AddVariable( "showerHits := hadronHits", &showerHits);
  if(piselvars[10]) _piTrackreader->AddVariable( "Range  := rangqP[0]*0.001", &rngMom);



  _electronShowerreader->AddSpectator( "nuEnergy  := NeuEng * 0.001", &nuEnergy);
  if(showerselvars[0]) _electronShowerreader->AddVariable( "ErrqP  := ErrMom[0]/RecMom[0]", &ErrqP);
  if(showerselvars[1]) _electronShowerreader->AddVariable( "trHits := InMu[0]", &trHits);
  if(showerselvars[2]) _electronShowerreader->AddVariable( "Rp := initrangqP[0]/RecMom[0]", &Rp);
  if(showerselvars[3]) _electronShowerreader->AddVariable( "engfrac := visEngTraj[0]/visibleEng", &engfrac);
  if(showerselvars[4]) _electronShowerreader->AddVariable( "meanDep := trajEngDep[0]==trajEngDep[0] ? trajEngDep[0] : 0.0", &meanDep);
  if(showerselvars[5]) _electronShowerreader->AddVariable( "EngVar := trajHighDep[0] > 0 && trajHighDep[0] == trajHighDep[0] ? trajLowDep[0]/trajHighDep[0] : 0.0", &EngVar);
  if(showerselvars[6]) _electronShowerreader->AddVariable( "recMom    := abs( 0.001/RecMom[0] )", &recMom);
  // if(showerselvars[7]) _electronShowerreader->AddVariable( "Qt  :=  abs( 0.001/RecMom[0] ) * ( 1 - pow( hadRdirP[0][0]*RecXDirection[0] + hadRdirP[0][1]*RecYDirection[0] + hadRdirP[0][2], 2)/(1 + pow(RecXDirection[0],2) + pow(RecYDirection[0],2)))", &Qt );
  if(showerselvars[7]) _electronShowerreader->AddVariable( "Qt  :=  abs( 0.001/RecMom[0] ) * ( 1 - pow(RecShowerDir[0]*RecXDirection[0] + RecShowerDir[1]*RecYDirection[0] + RecShowerDir[2], 2)/(1 + pow(RecXDirection[0],2) + pow(RecYDirection[0],2)))", &Qt );
  if(showerselvars[8]) _electronShowerreader->AddVariable( "meanHPP := Nallhits/Planes", &meanHPP);
  if(showerselvars[9]) _electronShowerreader->AddVariable( "showerHits := hadronHits", &showerHits);
  if(showerselvars[10]) _electronShowerreader->AddVariable( "Range  := rangqP[0]*0.001", &rngMom);
  ...
  TString piprefix = store.find_sstore("piTrainingLib") ?
    store.fetch_sstore("piTrainingLib") : "NC_Classification";
  TString showerprefix = store.find_sstore("showerTrainingLib") ?
    store.fetch_sstore("showerTrainingLib") : "NC_Classification";
  ...
  for (std::map<std::string,int>::iterator it = Use.begin(); it != Use.end(); it++) {
    if (it->second) {
      ...
      TString piweightfile = dir + piprefix + TString("_") + TString(it->first) + TString(".weights.xml");
      TString showerweightfile = dir + showerprefix + TString("_") + TString(it->first) + TString(".weights.xml");
      _reader->BookMVA( methodName, weightfile );
      _piTrackreader->BookMVA( methodName, piweightfile );
      _electronShowerreader->BookMVA( methodName, showerweightfile );
    }
  }
}

void Run_NDTMVA::define_histograms(const bhep::gstore& store) {
...
void Run_NDTMVA::CreateHistogram(std::string Key, int ntBins, double* recEdge, double* trEdge, 
  ...
  */
  ...
  hrecEffnDIS[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_muall_" + Key;
  hrecEffmuall[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_1mu_" + Key;
  hrecEff1mu[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_1mu1p_" + Key;
  hrecEff1mu1p[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_1mu1pi_" + Key;
  hrecEff1mu1pi[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_1mu1pi1p_" + Key;
  hrecEff1mu1pi1p[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_1mu2pi_" + Key;
  hrecEff1mu2pi[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_1mueshow_" + Key;
  hrecEff1mueshow[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_1muhshow_" + Key;
  hrecEff1muhshow[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_1muOther_" + Key;
  hrecEff1muOther[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);

  histname = "recEff_0mu_" + Key;
  hrecEff0mu[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_0mu1p_" + Key;
  hrecEff0mu1p[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_0mu1pi_" + Key;
  hrecEff0mu1pi[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_0mu1pi1p_" + Key;
  hrecEff0mu1pi1p[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_0mu2pi_" + Key;
  hrecEff0mu2pi[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_0mueshow_" + Key;
  hrecEff0mueshow[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_0muhshow_" + Key;
  hrecEff0muhshow[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
  histname = "recEff_0muOther_" + Key;
  hrecEff0muOther[Key] = new TH2D( histname.c_str(),
			      ";Reconstructed Energy (GeV);True Energy (GeV)",
			      ntBins+1, recEdge, ntBins, trEdge);
...
void Run_NDTMVA::FillHistogram(std::string Key, double Erec, double Etru,
...
void Run_NDTMVA::FillEventHistogram(std::string Key, double Erec, double Etru,
				    double qPrec, double qPtru, 
				    double cutvar, double limit, std::vector<int> npart){
  
  int q = qPrec != 0.0 ? int(qPrec/fabs(qPrec)) : 0;
  if(Erec > _maxE) Erec = _maxE;
  if(q == _beamCharge && cutvar > limit && npart[0] > 0){
    hrecEffmuall[Key]->Fill(Erec, Etru);
    if(npart[1] == 0 && npart[2] == 0)
      hrecEff1mu[Key]->Fill(Erec, Etru);
    else if(npart[2]==1 && npart[1]==0)
      hrecEff1mu1p[Key]->Fill(Erec, Etru);
    else if(npart[2]==0 && npart[1]==1)
      hrecEff1mu1pi[Key]->Fill(Erec, Etru);
    else if(npart[2]==1 && npart[1]==1)
      hrecEff1mu1pi1p[Key]->Fill(Erec, Etru);
    else if(npart[2]==0 && npart[1]==2)
      hrecEff1mu2pi[Key]->Fill(Erec, Etru);
    else if(npart[3]==1)
      hrecEff1mueshow[Key]->Fill(Erec, Etru);
    else if(npart[4]==1)
      hrecEff1muhshow[Key]->Fill(Erec, Etru);
    else
      hrecEff1muOther[Key]->Fill(Erec, Etru);
  } else {
    if(npart[2] == 0 && npart[1] == 0)
      hrecEff0mu[Key]->Fill(Erec, Etru);
    else if(npart[2]==1 && npart[1]==0)
      hrecEff0mu1p[Key]->Fill(Erec, Etru);
    else if(npart[2]==0 && npart[1]==1)
      hrecEff0mu1pi[Key]->Fill(Erec, Etru);
    else if(npart[2]==1 && npart[1]==1)
      hrecEff0mu1pi1p[Key]->Fill(Erec, Etru);
    else if(npart[2]==0 && npart[1]==2)
      hrecEff0mu2pi[Key]->Fill(Erec, Etru);
    else if(npart[3]==1)
      hrecEff0mueshow[Key]->Fill(Erec, Etru);
    else if(npart[4]==1)
      hrecEff0muhshow[Key]->Fill(Erec, Etru);
    else
      hrecEff0muOther[Key]->Fill(Erec, Etru);
  }
}


/********************************************/
void Run_NDTMVA::plotEventTopology(int ifig, int imu, std::string method, 
...
/********************************************/
void Run_NDTMVA::define_rec_binning(const bhep::gstore& store, double* recB, double* truB)
/********************************************/
{
...
/********************************************/
{
  ...
  theTree->SetBranchStatus( "rangqP", 1);          theTree->SetBranchAddress( "rangqP", &rangqP);
  ...
  for (Long64_t ievt=0; ievt<theTree->GetEntries()-1;ievt++) {
    ...
    */
    ...
    double muweight = 1.0, piweight = 1.0, showerweight = 1.0;
    double muweightcut = 0.5; //  piweightcut = 1.0, showerweightcut;
    std::vector<int> bestMuon;
    std::vector<double> bestWeight;
    
    for(int i=0; i<_myMethodList.size(); i++){
      bestMuon.push_back(-1);
      bestWeight.push_back(0.0);
    }
    for ( imu=0; imu < no_traj; imu++){
      if( !PassQualityCuts() ){
	continue;
      } 
      SetKinematics();
      for(int j=0; j<_myMethodList.size(); j++){
	std::string method = _myMethodList[j];
	muweight = _reader->EvaluateMVA( method+" method") ;
	if (muweight > bestWeight[j]){
	  bestWeight[j] = muweight;
	  bestMuon[j] = imu;
	}
      }
    }
    // Now need to re-evaluate the cuts for each best muon candidate.
    // Need to histogram the candidates to see if there is aggreement between methods.
    
    for(int i=0; i<_myMethodList.size(); i++){
      std::string method = _myMethodList[i];
      // set the muon index
      imu = bestMuon[i];
      // double check that the track passes cuts
      // and reset all selection and plotting variables.
      bool IsGoodMuon = true;
      std::vector<int> npart;
      for(int l=0; l<5; l++) npart.push_back(0);
      // int nmuon=0, nproton=0, npion=0, iselectronshow = 0, ishadronshow = 0;
      if ( imu >= 0){
	if ( PassQualityCuts() ) {
	  ...
	  std::string method = _myMethodList[i];
	  muweightcut = _MVAcut[i];
	  // Preliminaries to filling the histograms prior to event selection
	  // method = "No_MVA_" + method;      
	  muweight = 1.;
	  // FillHistogram(method, recEng, nuEnergy, qP[imu], qPtru[0], weight, weightcut, truInt); 
	  // Now evaluate the weight for the preferred track 
	  muweight = _reader->EvaluateMVA( method+" method") ;
	  if(muweight < muweightcut){ IsGoodMuon = false; }
	  else { npart[0]++; }
	  
	  FillHistogram(method, recEng, nuEnergy, qP[imu], qPtru[0], muweight, muweightcut, truInt); 
	}
      }
      else
	{ IsGoodMuon = false;}
      if ( no_traj > 0 ){
	for (int it=0;it<no_traj;it++)
	  { 
	    if (IsGoodMuon && it == bestMuon[i])
	      continue;
	    imu = it;
	    if ( PassQualityCuts() ) {
	      double weightcut = _piMVAcut[i];
	      // Preliminaries to filling the histograms prior to event selection
	      // method = "No_MVA_" + method;      
	      piweight = 1.;
	      // FillHistogram(method, recEng, nuEnergy, qP[imu], qPtru[0], weight, weightcut, truInt); 
	      // Now evaluate the weight for the preferred track 
	      piweight = _piTrackreader->EvaluateMVA( method+" method") ;
	      if (piweight<weightcut) npart[2]++;
	      else npart[1]++;
	    }
	  }
      }
      if ( showerHits > 10 && vertZ[0] < 0.0) {
	std::string method = _myMethodList[i];
	double weightcut = _showerMVAcut[i];
	// Preliminaries to filling the histograms prior to event selection
	// method = "No_MVA_" + method;      
	showerweight = 1.;
	// FillHistogram(method, recEng, nuEnergy, qP[imu], qPtru[0], weight, weightcut, truInt); 
	// Now evaluate the weight for the preferred track 
	showerweight = _electronShowerreader->EvaluateMVA( method+" method") ;
	if (showerweight<weightcut)  npart[3] = 1;
	else npart[4] = 1;
      }
	...
	imu = 0;
      ...
      // for(int l=0; l<5; l++) std::cout<<npart[l]<<std::endl;
      FillEventHistogram(method, recEng, nuEnergy, qP[imu], qPtru[0], muweight, muweightcut, npart); 
...
/*******************************************/
bool Run_NDTMVA::PassQualityCuts()
/*******************************************/
{

  if(fit[imu]==0) return false;
  if ( !fiducial2(no_traj)) return false;
  ...
  if(fabs(_beamCharge * 0.001/qP[imu]) > _maxOver * _maxE) return false;
  if(double(fitn[imu])/double(inmu[imu]) < _minNodes) return false;
...
/********************************************/
{
  ...
  while ( incr < alltracks2->GetXaxis()->GetXmax() )
    {
      alltracks2->Fill( incr, NeuEng * 0.001 );
...
/********************************************/
bool Run_NDTMVA::fiducial(double vx, double vy, double vz){
/********************************************/
...
/********************************************/
bool Run_NDTMVA::fiducial2(int nTraj) { 
/********************************************/
...
/********************************************/
bool Run_NDTMVA::pseudoTimeCut(std::vector<double> XPos,
...
/********************************************/
{
  ...
  for (int i=0; i<_myMethodList.size(); i++){
    ...
    */
    ...
    hrecEffmuall[_myMethodList[i]]->Sumw2();
    hrecEffmuall[_myMethodList[i]]->Write();
    hrecEff1mu[_myMethodList[i]]->Sumw2();
    hrecEff1mu[_myMethodList[i]]->Write();
    hrecEff1mu1p[_myMethodList[i]]->Sumw2();
    hrecEff1mu1p[_myMethodList[i]]->Write();
    hrecEff1mu1pi[_myMethodList[i]]->Sumw2();
    hrecEff1mu1pi[_myMethodList[i]]->Write();
    hrecEff1mu1pi1p[_myMethodList[i]]->Sumw2();
    hrecEff1mu1pi1p[_myMethodList[i]]->Write();
    hrecEff1mu2pi[_myMethodList[i]]->Sumw2();
    hrecEff1mu2pi[_myMethodList[i]]->Write();
    hrecEff1mueshow[_myMethodList[i]]->Sumw2();
    hrecEff1mueshow[_myMethodList[i]]->Write();
    hrecEff1muhshow[_myMethodList[i]]->Sumw2();
    hrecEff1muhshow[_myMethodList[i]]->Write();
    hrecEff1muOther[_myMethodList[i]]->Sumw2();
    hrecEff1muOther[_myMethodList[i]]->Write();

    hrecEff0mu[_myMethodList[i]]->Sumw2();
    hrecEff0mu[_myMethodList[i]]->Write();
    hrecEff0mu1p[_myMethodList[i]]->Sumw2();
    hrecEff0mu1p[_myMethodList[i]]->Write();
    hrecEff0mu1pi[_myMethodList[i]]->Sumw2();
    hrecEff0mu1pi[_myMethodList[i]]->Write();
    hrecEff0mu1pi1p[_myMethodList[i]]->Sumw2();
    hrecEff0mu1pi1p[_myMethodList[i]]->Write();
    hrecEff0mu2pi[_myMethodList[i]]->Sumw2();
    hrecEff0mu2pi[_myMethodList[i]]->Write();
    hrecEff0mueshow[_myMethodList[i]]->Sumw2();
    hrecEff0mueshow[_myMethodList[i]]->Write();
    hrecEff0muhshow[_myMethodList[i]]->Sumw2();
    hrecEff0muhshow[_myMethodList[i]]->Write();
    hrecEff0muOther[_myMethodList[i]]->Sumw2();
    hrecEff0muOther[_myMethodList[i]]->Write();
